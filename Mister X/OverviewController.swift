...
class OverviewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        Database.database().reference().child("game").observe(.childAdded, with: { (snap: DataSnapshot) in
            ...
            if snap.hasChild("isPublic") {
                if snap.childSnapshot(forPath: "isPublic").value as! Bool {
                    
                    if snap.hasChild("gameClosed") {
                        if !(snap.childSnapshot(forPath: "gameClosed").value as! Bool) {
                            
                            //check if the game is still running
                            if snap.hasChild("MisterX"){
                                ...
                                for t in snap.childSnapshot(forPath: "MisterX").children {
